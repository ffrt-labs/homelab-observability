#!/bin/sh
# The dead-man's switch, and the disk check.
#
# Both are INVERTED heartbeats: this script pings only while it can prove
# things are healthy, and it is the ABSENCE of a ping that raises the alarm
# via Healthchecks.io. Every failure path below therefore ends in "do not
# ping" — a check that pings on error is worse than no check at all,
...
log "canary starting: loki=$LOKI_URL window=$LOKI_WINDOW disk=$DISK_PATH threshold=${DISK_THRESHOLD_PCT}% interval=${CHECK_INTERVAL_SECONDS}s"
